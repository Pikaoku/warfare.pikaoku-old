...
app
  .storage()
  .refFromURL(
    "https://firebasestorage.googleapis.com/v0/b/pikaoku-tools.appspot.com/o/noun_wings_1113796.svg?alt=media&token=27c4eee5-dde0-424e-8696-b77c554055fa"
  );
...
const userSettings$ = authState(auth).pipe(
  filter(user => !!user),
  switchMap(user => doc(firestore.doc("users/" + user.uid)))
);

const COLLECTION_ARMIES = FIRESTORE_BASE_PATH + "armies";
...
// DATA STREAMS
const coreAspects$ = collectionData(
  firestore.collection(COLLECTION_ASPECTS).where("official", "==", true),
  "id"
);
const coreFeatures$ = collectionData(
  firestore.collection(COLLECTION_FEATURES).where("official", "==", true),
  "id"
);
...
const createObservableForUserCreatedData = collection =>
  authState(auth).pipe(
    ...
    switchMap(user =>
      collectionData(
        firestore.collection(collection).where("authorId", "==", user.uid),
        "id"
      )
    )
  );
    ...
    switchMap(user =>
      collectionData(
        firestore
          ...
          .where("saved", "array-contains", user.uid),
        "id"
      )
    )
  );

const userArmies$ = createObservableForUserCreatedData(COLLECTION_ARMIES);
